...
save([mypath 'G1props10.mat'],'myprops1')

for i=1:numel(ind2)
    runno=runnos{ind2(i)} ;
    myprops2(i,:,:)=ank_getvprops(mypath,runno,myroino);
end
save([mypath 'G2props10.mat'],'myprops2')
...
for j=1:14
    prefix=char(props{j});
    writemystats1(squeeze(myprops2(:,j,:))',squeeze(myprops1(:,j,:))', [mypath prefix '_groupstats2' suffix],norm); %feed treated in first
